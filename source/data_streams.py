...
#Gets Data from alphavantage.com
class get_data_tiingo:
    #Downloads a list of symbols for NYSE and NASDAQ you must specify which exchange you want
    def symbols_alpha(self,Exchange,start_date,end_date):
        if Exchange.lower() == 'nyse':
            #NYSE Symbols
            Symbols  = ticker_list(Exchange.lower()).values
        elif Exchange.lower() == 'nasdaq':
            #NASDAQ Symbols
            Symbols = ticker_list(Exchange.lower())
        elif Exchange.lower() == 'amex':
            Symbols = pandas.read_csv(AMEXtickerlist)['Symbol'].values
        else:
            #If anything other than the valid exchanges defined in the if and elif statements above
    ...
    def NYSEandNASDAQData(self):
       ...
       # AMEX = GetData.symbols_alpha(self,'AMEX')
        ...
        NYSE = get_data_tiingo.symbols_alpha(self, 'NYSE')
        # Get Data for NASDAQ by specifying the exchange when call the function
        NASDAQ = get_data_tiingo.symbols_alpha(self, 'NASDAQ')
...
class get_data_a_v:
    #Downloads a list of symbols for NYSE and NASDAQ you must specify which exchange you want
    def symbols_alpha(self,Exchange):
        if Exchange.lower() == 'nyse':
            #NYSE Symbols
            t_symbols  = ticker_list(Exchange.lower())
            Symbols = []
            for x in t_symbols:
                Symbols.append(x[0])
        elif Exchange.lower() == 'nasdaq':
            ...
            try:
                ...
                while True:
                    ...
                    try:
                        ...
                        if datetime.now() - callstart < timedelta(seconds=4.1):
                            Delta = ((callstart+timedelta(seconds=4.1))-datetime.now()).total_seconds()
                            time.sleep(Delta)
                        callstart = datetime.now()
                        Data = pandas.read_csv(url)
                    except URLError:
                ...
                if Data.values[0][0] != '    "Error Message": "Invalid API call. Please retry or visit the documentation (https://www.alphavantage.co/documentation/) for TIME_SERIES_DAILY_ADJUSTED."':
                    ...
                    Data.rename(columns={'timestamp':'dat'},inplace=True)
                    #Renaming adjusted_close to adj_close to keep data generic with other sources
    ...
    def NYSEandNASDAQData(self):
       ...
       # AMEX = GetData.symbols_alpha(self,'AMEX')
        #Get Data for NYSE by specifying the exchange when call the function
        NYSE = get_data_a_v.symbols_alpha(self, 'NYSE')
        # Get Data for NASDAQ by specifying the exchange when call the function
        NASDAQ = get_data_a_v.symbols_alpha(self, 'NASDAQ')
        #Create a Dataframe that will have both exchange's data by setting it equal to NYSE
        Overall = NYSE
        #Append NASDAQ Data to the dataframe created above
        Overall = Overall.append(NASDAQ)
