...
from data_scripts.ticker_list_streams import get_tickers_wikipedia
...
while True:
    ...
    print('Updating Ticker List from wikipedia on', Today)

    #Get Tickers

    tickers = get_tickers_wikipedia()

    #upload to rds

    upload_to_rds_table(tickers,'wikipedia',schema='ticker_lists',index=['ticker','exchange'],row_by_row=True,save_errors=True)

    now = datetime.now()
    # Get the time we want to re-run the program at
    runAt = (datetime.now() + timedelta(days=1)).replace(hour=17, minute=0, second=0, microsecond=0)
