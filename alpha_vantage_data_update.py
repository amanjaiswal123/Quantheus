...
from source.Commons import TradingDays, NearestTradingDay
import os
...
while True:
    ...
    recent_date_rds = get_max_date('alpha_vantage')
    ...
    if recent_trading_day != recent_date_rds:
        print('Starting Data Pull from', start_date, 'to', recent_trading_day)
        try:
            todaydata = alphavantage_nyse_nasdaq_download(end_date=recent_trading_day, start_date=start_date)
            ...
            raise e
        upload_to_rds_table(todaydata,'alpha_vantage',row_by_row=False,save_errors=True)
    now = datetime.now()
