...
def UpTrend(Fields, Portfolio, aval_cash, net_worth, datadate, first_calc, YSignals=None):
    ...
    Pipe = pandas.DataFrame(data={"Today's Close":TodaysClose,"Today's Adjusted Close":TodaysAdjustedClose,'Action':'Nothing','Holding Period':'Infinite'})
    ...
    BuyFilter = (Pipe['Peak Trough Cycles'] >= 3) & (Pipe['Recent Peak or Trough'] == 'Trough') & (Pipe["Today's Adjusted Close"] >= Pipe['Trough']*1.05) & (Pipe['Cycle Position'] > .05) & (Pipe['RSI'] >= 55) & (Pipe['Potential Gain'] >= .05) & (Pipe["Today's Adjusted Close"] >= 1) & (Pipe['90 Day Average Volume'] >= 50000) & (Pipe['Average Peak Trough Turnover'] <= 900) & (Pipe['Current Cycle Duration'] >= 180) & (Pipe['Gain to Loss Ratio'] >= 1) & (Pipe['Past Support or Ressitance'] >= .02) & (Pipe["Today's Adjusted Close"] <= 30) & (Pipe["Total Distance"] <= 2.5)
    SellFilter = (Pipe['Trough'].isna()) | (Pipe['Peak'].isna()) | (Pipe["Today's Adjusted Close"] >= Pipe['Peak']*.9)
    Pipe['Stop Loss'] = numpy.nan
    Pipe['Stop Loss'][BuyFilter & ((Pipe["Trough"]*.9) > (Pipe["Today's Adjusted Close"] * .7))] = Pipe["Trough"]*.9
    Pipe['Stop Loss'][BuyFilter & ((Pipe["Today's Adjusted Close"] * .7) >= Pipe["Trough"]*.9)] = Pipe["Today's Adjusted Close"] * .7
    ...
    return Pipe[BuyFilter & SellFilter]
 #Realtime(UpTrend,365*5)
Backtest(UpTrend,End_Date='2017-01-01',Start_Date='2014-01-01',StartingOffset=365*5,SC=200000,marg_call_protect=True,MaintenceMarginRequirment=1.5,BacktestTitle='PeakTroughP60MKSTPLMTMargin')
