...
datadate = _datadate()
Fields = AHistroy('all','backtest',End_Day=datadate,Days=365*2.5)
...
Data90 = datadateslice(Fields['adj_close'],End_Date=datadate,Days=365*2)
...
BuyFilter = (Pipe['Daily PCT Change'] >= .005) & (Pipe['Daily PCT Change'] <= .1) & (Pipe['Rally Duration'] >= 10) & (Pipe['Price Position'] >= 0) & (Pipe['Rally Linear Growth Rate'] >= .003) & (Pipe['Recent Peak or Trough'] == 'Trough') & (Pipe['Potential Gain'] >= .05) & (Pipe['90 Day Average Volume'] >= 20000) & (Pipe['Past Support or Ressitance'] <= .08) & (Pipe['Total Size'] >= 200) & (Pipe['Peak Trough Cycles'] >= 3) & (Pipe["Today's Adjusted Close"] >= 1)
pass