...
def UpTrend(Fields, Portfolio, datadate, firstcalc, YSignals=None):
    ...
    if firstcalc:
        ...
        RSI = _RSI(data30,datadate,firstcalc,Period=60)
        ...
        RSI = _RSI(data2,datadate,firstcalc,prevgain,prevloss,Period=60)
    ...
    if len(Portfolio.index) > 0:
        if 'Peak on Buy' in Portfolio.columns.values:
            Pipe['Peak'][(Pipe['Peak'].isna()) & InPortfolio] = Portfolio['Peak on Buy'][(Pipe['Peak'][InPortfolio].isna())]
        if 'Trough on Buy' in Portfolio.columns.values:
            Pipe['Trough'][(Pipe['Trough'].isna()) & InPortfolio] = Portfolio['Trough on Buy'][(Pipe['Trough'][InPortfolio].isna())]
    if firstcalc is False:
...
#Backtest(UpTrend,End_Date='2017-01-01',Start_Date='2014-01-01',StartingOffset=365*5,SC=1000000,BacktestTitle='PeakTroughP60MKSTPLMT')
